...
def train_and_evaluate(net, dataloaders, config, device, lin_cls=False):
    ...
    def train_step(net, sample, loss_fn, optimizer, device, loss_input_fn):
        ...
        outputs = net(x=sample['inputs'].to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', default='./configs/PASTIS24/TSViT_seg.yaml', help='configuration (.yaml) file to use')
    ...
    config["DATASETS"]["train"]["pseudo_path"] = args.pseudo_path
