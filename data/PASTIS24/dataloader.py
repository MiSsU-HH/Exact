...
def get_dataloader(paths_file, root_dir, transform=None, batch_size=32, num_workers=4, shuffle=True,
                   return_paths=False, my_collate=None, pseudo_path=False, is_dist=False):
    dataset = SatImDataset(csv_file=paths_file, root_dir=root_dir, transform=transform, return_paths=return_paths, pseudo_path=pseudo_path)
...
class SatImDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, transform=None, multilabel=False, return_paths=False, pseudo_path=False):
        """
        Args:
