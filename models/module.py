...
class LocalConsistencyModule(nn.Module):
    ...
    def forward(self, imgs, pred_phis):
        ...
        # aff = aff.mean(dim=1, keepdim=True) ## weight of time
    ...
    def forward_weighted(self, imgs, pred_phis, weights):
        # weights = weights.to(pred_phis.device) # B Category T
        _imgs = self.get_dilated_neighbors(imgs) # B T*C 8 H W
        _imgs_rep = imgs.unsqueeze(2).repeat(1, 1, _imgs.shape[2], 1, 1) # B T*C 8 H W
        
        _imgs_abs = torch.abs(_imgs - _imgs_rep) # B T*C 8 H W
        _imgs_std = torch.std(_imgs, dim=2, keepdim=True) # B T*C 1 H W

        aff = -(_imgs_abs / (_imgs_std + 1e-8) / self.alpha) ** 2 # B T*C 8 H W

        aff = aff.reshape(aff.shape[0],aff.shape[1]//10,10,aff.shape[2],aff.shape[3],aff.shape[4]).unsqueeze(1).repeat(1,weights.shape[1],1,1,1,1,1)
        # aff = aff.mean(dim=1, keepdim=True) ## weight of time
        try:
            weights = weights[:,:,:aff.shape[2]].unsqueeze(3).unsqueeze(4).unsqueeze(5).unsqueeze(6)
        except:
            debug = True
        else:
            aff = aff * weights
